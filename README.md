...
- 🎯 Entry points to execute your program using `python -m <python_tp>` or `$ python_tp` with basic CLI argument parsing.
...
[![codecov](https://codecov.io/gh/ctfree/python_tp/branch/main/graph/badge.svg?token=python_tp_token_here)](https://codecov.io/gh/ctfree/python_tp)
[![CI](https://github.com/ctfree/python_tp/actions/workflows/main.yml/badge.svg)](https://github.com/ctfree/python_tp/actions/workflows/main.yml)

Awesome python_tp created by ctfree

## Install

from source
```bash
git clone https://github.com/ctfree/python_tp python_tp
cd python_tp
make install
```
...
```bash
pip install python_tp
```

## Executing

```bash
$ python_tp run --port 8080
```

or

```bash
python -m python_tp run --port 8080
```

or

```bash
$ uvicorn python_tp:app
```
...
```bash
❯ python_tp --help
Usage: python_tp [OPTIONS] COMMAND [ARGS]...
...
```bash
❯ python_tp create-user --help
Usage: python_tp create-user [OPTIONS] USERNAME PASSWORD
...
```bash
python_tp create-user admin admin --superuser
```
...
Run with `python_tp run` and access http://127.0.0.1:8000/docs

![](https://raw.githubusercontent.com/rochacbruno/fastapi-project-template/master/docs/api.png)


**For some api calls you must authenticate** using the user created with `python_tp create-user`.
...
-----------------------------------------------------
python_tp/__init__.py              4      0   100%
python_tp/app.py                  16      1    94%
python_tp/cli.py                  21      0   100%
python_tp/config.py                5      0   100%
python_tp/db.py                   10      0   100%
python_tp/models/__init__.py       0      0   100%
python_tp/models/content.py       47      1    98%
python_tp/routes/__init__.py      11      0   100%
python_tp/routes/content.py       52     25    52%
python_tp/routes/security.py      15      1    93%
python_tp/routes/user.py          52     26    50%
python_tp/security.py            103     12    88%
-----------------------------------------------------
TOTAL                               336     66    80%
...
```py
from python_tp.config import settings
```
...
> `dynaconf_merge` is a boolean that tells if the settings should be merged with the default settings defined in python_tp/default.toml.

### As environment variables
```bash
export python_tp_KEY=value
export python_tp_KEY="@int 42"
export python_tp_KEY="@jinja {{ this.db.uri }}"
export python_tp_DB__uri="@jinja {{ this.db.uri | replace('db', 'data') }}"
```
...
```bash
python_tp_ENV=production python_tp run
```
